...
def discover(
    science_dir: str = typer.Option("~/notes/300-reference/science", "--dir", "-d"),
    min_notes: int = typer.Option(3, "--min", help="Min # of your notes that must cite a work"),
    top: int = typer.Option(40, "--top", "-n", help="How many recommendations per signal"),
    offline: bool = typer.Option(False, "--offline", help="Signal A only (skip OpenAlex B/C)"),
    out: str = typer.Option(
        "~/notes/300-reference/science/READING-LIST.md", "--out", "-o",
        help="Where to write the reading list note",
    ),
):
    """Build a reading list of papers you should read — a LIST for review only.

    Signal A (self-contained): co-citation gaps — works cited by many of your notes
    that you don't own. Signal B (OpenAlex): recent papers that CITE your key papers.
    Signal C (OpenAlex): papers matching your top method×system facets. Adds NOTHING
    to Zotero — you approve each paper yourself.
    """
    ...
    lines = [
        "---", "type: reading-list", "tags: [reading-list]", "---",
        "# Reading list — papers to consider", "",
        f"*Generated by `zot discover` from {len(list(sdir.glob('*.references.txt')))} notes. "
        "**Review only — nothing was added to Zotero.** Tick a box, then add the ones you want.*", "",
    ]

    # Signal A
    gaps = disc.cocitation_gaps(sdir, lib, min_notes=min_notes)
    rprint(f"[bold]A:[/bold] {len(gaps)} works cited by ≥{min_notes} of your notes, not owned.")
    lines += ["## A — cited by many of your papers (co-citation gaps)", "",
              "| ✓ | cited by | paper | your notes |", "|---|---|---|---|"]
    for g in gaps[:top]:
        ref = g["ref"].replace("|", "\\|")
        ref = ref[:157] + "…" if len(ref) > 160 else ref
        cited = ", ".join(f"[[{s}]]" for s in g["notes"][:3]) + (f" +{len(g['notes'])-3}" if len(g["notes"]) > 3 else "")
        lines.append(f"| [ ] | **{g['count']}×** | {ref} | {cited} |")
    lines.append("")

    if not offline:
        key = [it for it in lib if "status/key-paper" in it.tags or "type/my-paper" in it.tags]
        # Signal B
        rprint(f"[bold]B:[/bold] querying OpenAlex forward-citations for {len(key)} key papers…")
        fc = disc.forward_citations(lib, key, per_paper=40)
        lines += ["## B — recent papers that cite your key papers (forward citations)", "",
                  "| ✓ | cites | year | paper | venue |", "|---|---|---|---|---|"]
        for r in fc[:top]:
            title = r["title"].replace("|", "\\|")[:150]
            link = f"[{title}](https://doi.org/{r['doi']})" if r["doi"] else title
            lines.append(f"| [ ] | {len(r['via'])}× | {r['year']} | {link} | {r['venue'][:30]} |")
        lines.append("")
        rprint(f"   {len(fc)} candidates")
        # Signal C
        combos = disc.top_facet_combos(sdir, n=6)
        rprint(f"[bold]C:[/bold] OpenAlex facet search for {len(combos)} top method×system combos…")
        fs = disc.facet_search(lib, combos, per_combo=15)
        lines += ["## C — papers matching your top method×system facets", "",
                  f"*Combos: {', '.join(f'{m}×{s}' for m,s in combos)}*", "",
                  "| ✓ | year | paper | venue |", "|---|---|---|---|"]
        for r in fs[:top]:
            title = r["title"].replace("|", "\\|")[:150]
            link = f"[{title}](https://doi.org/{r['doi']})" if r["doi"] else title
            lines.append(f"| [ ] | {r['year']} | {link} | {r['venue'][:30]} |")
        lines.append("")
        rprint(f"   {len(fs)} candidates")

    outp = Path(out).expanduser()
    outp.write_text("\n".join(lines) + "\n")
    rprint(f"\n[green]wrote[/green] {outp}  — open it in Obsidian to review + tick papers to add.")
