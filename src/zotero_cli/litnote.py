...
def build_mocs(science_dir: Path) -> list[str]:
    ...
    # Facet index MOCs: group the corpus by the fine-grained methods_used /
    # systems_used frontmatter arrays. FLATTEN is required — GROUP BY on a list
    # field would group by the whole array, not per-facet.
    for key, title in (("methods_used", "Methods index"), ("systems_used", "Systems index")):
        slug = f"MOC - {key.replace('_used', '')}-index"
        body = (
            f"---\ntype: moc\ntags: [moc]\nzot_built: '2026-07-04'\n---\n"
            f"# {title}\n\nPapers grouped by fine-grained `{key}` facet "
            f"(agent-extracted, canonical). Auto-generated by `zot moc-sync`.\n\n"
            f"```dataview\nTABLE rows.file.link AS Papers\n"
            f'FROM "300-reference/science"\nWHERE {key}\n'
            f"FLATTEN {key} AS facet\nGROUP BY facet\nSORT length(rows) DESC\n```\n"
        )
        (mocdir / f"{slug}.md").write_text(body)
        written.append(slug)
    return written
